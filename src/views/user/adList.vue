			...
			<el-form :model="form"  :rules="rules" ref="ruleForm">		
				<div class="innerInput filePs">
					<el-form-item label="附件" >
					   <el-input v-model="fileName" clearable autocomplete="off"></el-input>
					</el-form-item> 
					<el-upload
						ref="upload"
					  class="upload-demo"
					  action="http://ts-i.idatachain.cn/api/upload/attachment"					  
					  :limit="1"
					  :name="'file'"
					  :data="uerAd"
					  :on-success="onsuc"
					...
					<div v-if="tabsd.flow_detail && tabsd.flow_detail.fields && tabsd.flow_detail.type == 'field_edit'">
						...
						<el-form-item v-for="(el,index) in tabsd.flow_detail.fields"  v-if="el.type!='attachment'" :label="el.field">
						   <el-input v-if="el.type=='text' || el.type=='number'" v-model="form[el.name]" autocomplete="off"></el-input>
						   <el-select v-if="el.type =='select'" v-model="form[el.name]" placeholder="请选择">
						   		<el-option
						    ...
						    <el-checkbox-group   v-model="form[el.name]">
						   		<el-checkbox v-if="el.type =='checkbox'" v-for="(el2,index2) in el.options" :label="el2.label" :value="el2.value"></el-checkbox>							
						   </el-checkbox-group>
						   <el-date-picker v-if="el.type=='date'"
...
export default {
	...
	methods: {
		...
		pushFor(){			
			...
			this.api.handle_pass(datas).then((data)=>{	
				this.closeFor();
			});		
		},
		enloding(){
		...
		console.log(this.tableData[on].put_id);
