...
export default {
	...
	methods: {
		seeXx2(on){
			console.log(encodeURI(this.tableData[on].admaster))
		
			window.open('/#/data/ad_position_users?admaster='+encodeURI(this.tableData[on].admaster))
		},
		lodingfalse(){
		...
		getData(sxtj){
			...
			this.api.data_ad_master({params}).then((datas)=>{									
