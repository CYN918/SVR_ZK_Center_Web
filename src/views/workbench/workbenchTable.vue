                ...
                <el-table
                    ...
                    <el-table-column label="操作"> 
                        <template slot-scope="props">
                            ...
                            <el-button @click="check(tableData[props.$index].demand_type,tableData[props.$index].did,tableData[props.$index].status+1,tableData[props.$index].reject)" v-if="tableData[props.$index].reject=='1'&&tableData[props.$index].isfinish!=2&&tableData[props.$index].demand_type!='设计师结算'">查看驳回原因</el-button>
                            ...
                            <el-button  @click="getBH(props.$index)" v-if="tableData[props.$index].status_name!='完成投放'&&tableData[props.$index].status_name!='需求发布'&&tableData[props.$index].status_name!='提现审核'&&tableData[props.$index].status_name!='签字审核'&&tableData[props.$index].status_name!='结算汇款'&&tableData[props.$index].status_name!='补充签字'&&tableData[props.$index].status_name!='素材入库'&&tableData[props.$index].status_name!='确认入库'&&tableData[props.$index].status_name!='结算完成'&&tableData[props.$index].status!=1&&tableData[props.$index].emails.indexOf(email)!=-1&&tableData[props.$index].demand_type!='收款结算'&&tableData[props.$index].demand_type!='付款结算'&&tableData[props.$index].demand_type!='分成金额审核'&&tableData[props.$index].isfinish!=2">驳回</el-button>
                            ...
                            <el-button v-if='tableData[props.$index].demand_type=="分成金额审核"&&(tableData[props.$index].status==2||tableData[props.$index].status==3)&&tableData[props.$index].reject!=1' @click='CKdetails(tableData[props.$index])'>查看详情</el-button>
                            ...
                            <div>
                                <div class="step_box" v-for="(item,index) in tableData[props.$index].audit_process">
                                   ...
                                   <div class="step_contnet" v-if="(item.creator!=''||tableData[props.$index].status==item.status)&&((tableData[props.$index].demand_type=='收款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='付款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='业务需求')||(tableData[props.$index].demand_type=='素材需求')||(tableData[props.$index].demand_type=='设计师结算')||(tableData[props.$index].demand_type=='分成金额审核'))">
                                        ...
                                        <span v-if="item.isfinish==1&&item.status_name!='提现完成'&&item.status_name!='素材入库'">已完成</span>
                                        ...
                                        <span v-if="tableData[props.$index].status==item.status&&item.isfinish!='1'&&item.status_name!='素材入库'&&item.isfinish!=2&&tableData[props.$index].isfinish!='2'">待处理</span>
                                       <span v-if="(tableData[props.$index].status==item.status&&item.isfinish!='1'&&item.status_name!='素材入库'&&item.isfinish==2)||tableData[props.$index].isfinish=='2'">已作废</span>
                                    </div>
                                    <div class="step_contnet" v-if="(item.creator!=''||tableData[props.$index].status==item.status)&&((tableData[props.$index].demand_type=='收款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='付款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='业务需求')||(tableData[props.$index].demand_type=='素材需求')||(tableData[props.$index].demand_type=='设计师结算')||(tableData[props.$index].demand_type=='分成金额审核'&&item.status!=3))">
                                    ...
                                    <div class="step_contnet" v-if="(item.creator!=''||tableData[props.$index].status==item.status)&&((tableData[props.$index].demand_type=='收款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='付款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='业务需求')||(tableData[props.$index].demand_type=='素材需求')||(tableData[props.$index].demand_type=='设计师结算')||(tableData[props.$index].demand_type=='分成金额审核'&&item.status!=3))">
                                        ...
                                        <span class="dj" v-if="tableData[props.$index].demand_type=='设计师结算'&&(item.status=='5'||(item.status==4&&!item.attach))">已完成</span>
                                        <span class="dj" v-if="item.key==0&&item.status_name!='结算汇款'&&item.status_name!='确认入库'&&item.status_name!='物料审核'&&item.status_name!='测试验收'&&item.status_name!='发布审核'&&item.status_name!='项目发布'&&item.status_name!='素材入库'&&item.status_name!='提现完成'&&item.status_name!='素材入库'&&tableData[props.$index].demand_type!='收款结算'&&tableData[props.$index].demand_type!='付款结算'&&tableData[props.$index].demand_type!='分成金额审核'&&tableData[props.$index].demand_type!='设计师结算'&&item.reject!='1'&&item.isfinish!=2" @click="check(tableData[props.$index].demand_type,tableData[props.$index].did,item.status,item.reject)">查看详情</span>
                                        <span class="dj" v-if='tableData[props.$index].demand_type=="分成金额审核"&&item.did!=undefined&&item.reject!=1&&item.status==1'  @click='CKdetails(tableData[props.$index])'>查看详情</span>
                                         <span class="dj" v-if='tableData[props.$index].demand_type=="分成金额审核"&&item.did!=undefined&&item.reject!=1&&item.status==2' >审核通过</span>
                                        <span v-if="tableData[props.$index].status==item.status&&item.key!=0&&((tableData[props.$index].demand_type=='收款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='付款结算'&&item.status!=6)||(tableData[props.$index].demand_type=='设计师结算'&&item.key!=0&&item.status!=5))&&item.isfinish!=2">待处理</span>
                                        ...
                                        <span v-if="(tableData[props.$index].demand_type=='业务需求'&&item.status=='4'&&item.key==0&&item.reject!='1'&&item.isfinish!=2)">审核通过</span>
                                        <span class="dj" @click="check(tableData[props.$index].demand_type,tableData[props.$index].did,item.status,item.reject)"  v-if="item.reject=='1'&&tableData[props.$index].status!=item.status&&tableData[props.$index].demand_type!='分成金额审核'">查看驳回原因</span>
                                        <span class="dj" @click="check(tableData[props.$index].demand_type,tableData[props.$index].did,tableData[props.$index].status,item.reject)"  v-if="item.reject=='1'&&tableData[props.$index].status==item.status&&tableData[props.$index].demand_type!='分成金额审核'">查看驳回原因</span>
                                        <span v-if="(tableData[props.$index].demand_type=='素材需求'&&item.status=='2'&&item.key==0&&item.reject!='1'&&item.isfinish!=2)" >审核通过</span>
                                        ...
                                        <span class="dj" v-if="item.reject=='1'&&tableData[props.$index].demand_type=='分成金额审核'">审核不通过</span>
                                        <a  :href="item.attach.url"  class="dj"  v-if="(tableData[props.$index].demand_type=='设计师结算'&&(item.status=='2'||item.status=='4')&&item.key==0&&item.reject!='1'&&item.isfinish!=2&&item.attach)">下载查看</a>
                                    </div>
                                </div>
                            </div>
                        </template>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            //     this.move()
            // },
            withdraw(id,status,emails,lineStatus){
                this.$router.push({
                    query:{
                        id:id,
                        status:status,
