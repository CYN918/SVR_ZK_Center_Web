                            ...
                            <div>
                                <div class="step_box" v-for="(item,index) in tableData[props.$index].audit_process">
                                    ...
                                    <div class="step_contnet" v-if="item.creator!=''||tableData[props.$index].status==item.status">
                                        ...
                                        <span class="dj" v-if="item.key==0&&item.status_name!='结算汇款'&&item.status_name!='素材审核'&&item.status_name!='物料审核'&&item.status_name!='测试验收'&&item.status_name!='发布审核'&&item.status_name!='活动发布'&&item.status_name!='素材入库'&&item.status_name!='提现完成'&&item.status_name!='素材准备'" @click="check(tableData[props.$index].demand_type,tableData[props.$index].did,item.status,item.reject)">查看详情</span>
                                        ...
                                        <span class="dj" v-if="(tableData[props.$index].demand_type=='业务需求'&&item.status=='4'&&item.key==0)||(tableData[props.$index].demand_type=='业务需求'&&item.status=='2'&&item.key==0)||(tableData[props.$index].demand_type=='素材需求'&&item.status=='2'&&item.key==0&&item.reject!=1)">审核通过</span>
