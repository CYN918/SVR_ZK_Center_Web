	...
	<div class="tbale">
		<template v-if="typeOnde==0" class="tablboxd">
			<el-table :data="tableDatas" :summary-method="getSummaries" :show-summary="tableConfig['show-summary']" v-loading="loading" style="width: 100%"  @selection-change="handleSelectionChange">
				<el-table-column
				  type="selection"
				  width="55" v-if="tableConfig.ischeck">
				</el-table-column>
				<el-table-column v-if="tableConfig.expand" type="expand">
				  <template slot-scope="props">
						...
						<table class="mytabled" width="100%">
							<tr>
								<th v-for="(el,index) in tableConfig.expand">{{el.lable}}</th>
							</tr>
							<tr  v-for="(el,indexx) in props.row.works">
								<td v-for="(el2,index2) in tableConfig.expand">	
									<span v-if="el2.temps" class="el-button el-button--text bjysdicon iconfont"	 @click="clickfn('downlods',{on:props.$index,ons:indexx})">{{el2[0]}}&#xe61a</span>
				...
				<el-table-column  show-overflow-tooltip :width="el.widht" v-for="(el,index) in tableConfig.list"
				 :prop="el.prop" :label="el.lable" :sortable="el.sor">							 
					<template  slot-scope="scope" >
						...
						<el-button 
						v-else-if="el.temps" 
						v-for="el2 in el.temps"						
						@click="clickfn(el2.fnName,scope.$index)" 
						:class="el2.cls" 
						:type="el2.type" :size="el2.size" :round="el2.round" v-html="el2.value">						
						</el-button>
						<span 
						v-else-if="el.poclick" 
						@click="clickfn(el.poclick.fnName,scope.$index)" 
						:class="['jscbtn',el.poclick.cls]" 
						>	
							{{scope.row[el.prop]}}				
						</span>
						<el-switch v-else-if="el.switch" @change="clickfn(el.switch.fnName,scope.$index)"
						  v-model="el.switch.mode[scope.$index]"
						  active-color="#FFA39E"
						  inactive-color="#CFCDCD">
						</el-switch>
		...
		<ul v-if="typeOnde==1" class="tbalcp">
			<li v-for="(el,index) in tableDatas">
		...
		<div :class="['tableBuee',tableConfig.total==-1?'noeflow':'']" v-if="tableConfig.btns"  >
			<el-button v-for="(el,index) in tableConfig.btns" @click="clickfn(el.fnName)" :class="[el.cls,el.check?dcan:'']" round>{{el.value}}</el-button>
		</div>
