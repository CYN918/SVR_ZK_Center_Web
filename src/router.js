...
mode['user'] = userIndex
import userNeed from './views/user/needList.vue'
mode['user_need'] = userNeed
import userPicture from './views/user/adList.vue'
mode['user_picture'] = userPicture
mode['user_resource'] = userPicture
mode['user_wallpaper'] = userPicture
...
let wb = [
	...
	router.addRoutes(wb);
		let nb = [{path:'/',name:'首页',component:Index},
...
router.beforeEach((to, from, next) => {
	...
	let cent = 'center';
	let urld = 'http://c.zookingsoft.com/api/login';
...
// 				console.log(msg.data.data.user.leftNav);
// 				console.log(JSON.parse(msg.data.data.user.leftNav));
// 				localStorage.setItem('letNav',JSON.stringify(JSON.parse(msg.data.data.user.leftNav)));
// 				console.log(4);
// 				localStorage.setItem('wbdd',JSON.stringify(JSON.parse(msg.data.data.user.wb)));	
// 				console.log(5);
// 					let p = JSON.parse(msg.data.data.user.wb);
// 								
// 					if(p){
// 						
// 						back(p);
...
// 								data[i].component = mode[data[i].component[0]];		
// 								if(data[i].children){
// 									for(let j=0,n2=data[i].children.length;j<n2;j++){
// 										data[i].children[j].component = mode[data[i].children[j].component[0]];		
// 									}
// 								}
// 							}		
// 						}
...
// 				localStorage.setItem('letNav',JSON.stringify(leftNav));
// 				localStorage.setItem('nbnav',JSON.stringify(nb));
		...
		/*center_local center_dev center*/
	}
